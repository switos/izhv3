...
public class Gun : MonoBehaviour
{
    ...
    /// </summary>
    public float shotgunSpread = 60.0f;
    ...
    public void ShootGun(Transform director)
    {
        ...
        if(shotgun) {
            ...
            {
                float magicNumber = shotgunSpread / shotgunBullets;
                float angle = (-1*(shotgunSpread/2)) + ((magicNumber * i) + (magicNumber/2));

                Quaternion rotation = Quaternion.Euler(director.eulerAngles)* Quaternion.AngleAxis(angle, Vector3.forward);


                SpawnBullet(director.position, rotation);
            }
        } else {
            SpawnBullet(director.position, Quaternion.Euler(director.eulerAngles));
        }
